...
export class VehiculosModel {
    ...
    static editarVehiculo = async (vehiculo: Vehiculo, idVehiculo: number) => {
        try {
            const editarVehiculo = await pool.query(
                `
                UPDATE vehiculo SET dominio = $1, marca = $2, anio = $3, kilometraje = $4, id_empleado = $5, estado = $6 WHERE vehiculo.id = $7
                `,
                [
                    vehiculo.dominio,
                    vehiculo.marca,
    ...
    static eliminarVehiculo = async (idVehiculo: number) => {
        try {
            ...
            const eliminarRelacion = await ViajeModel.eliminarViaje(idVehiculo,'id_vehiculo')
